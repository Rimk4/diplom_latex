...
\begin{algorithm}
\SetAlgoLined
\KwData{Сеть $G=(V,E)$, узел-источник $s$, узел-назначения $d$}
...
\While{$RT$ не содержит маршрут к $d$}{
    Выбрать промежуточный узел $v$ с максимальной $q_i$\;
    Обновить $RT$ через $v$:\;
...
\begin{algorithm}
\SetAlgoLined
\KwData{Текущие координаты узлов $\mathbf{x}_t$, скорости $\mathbf{v}_t$}
...
\caption{Прогнозирование позиций узлов}\label{alg:position-prediction}
\end{algorithm}
...
\item 120 - остаточная энергия узла (мДж)
\item 150 - начальная энергия узла (мДж)
\end{itemize}